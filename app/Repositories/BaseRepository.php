...
class BaseRepository
{
    ...
    public function createPivot($model, array $payload = [], string $relation = ''){
        $id = $payload['language_id'] ?? $payload['attribute_id'] ?? $payload['post_id'] ?? $payload['product_id'] ?? $payload['amenity_id'] ?? $payload['amenity_catalogue_id'] ?? $model->id;
        return $model->{$relation}()->attach($id, $payload);
    }
